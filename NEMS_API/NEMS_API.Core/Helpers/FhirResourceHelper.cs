...
namespace NEMS_API.Core.Helpers
{
    public class FhirResourceHelper : IFhirResourceHelper
    {
        ...
        private IResourceResolver GetResolver()
        {
            ...
            return new CachedResolver(new MultiResolver(new DirectorySource(local, new DirectorySourceSettings { IncludeSubDirectories = true }), new WebResolver(uri => new FhirClient("https://fhir.hl7.org.uk/STU3")), new WebResolver(uri => new FhirClient("https://fhir.nhs.uk/STU3")), new ZipSource(zip)));
        }
    }
}
