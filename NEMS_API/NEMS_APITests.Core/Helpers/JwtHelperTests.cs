...
namespace NEMS_APITest.Core.Helpers
{
    public class JwtHelperTests : IDisposable
    {
        ...
        public void JWT_Check_ValidToken(string token)
        {
            ...
            var actual = jwtHelper.IsValid(token, new string[] { $"patient/DocumentReference.{JwtScopes.Write.ToString().ToLowerInvariant()}" }, issued);
        ...
        public void JWT_Check_InvalidScope()
        {
            ...
            var actual = jwtHelper.IsValid(token, new string[] { JwtScopes.Read.ToString() }, issued);
        ...
        public void JWT_Check_InvalidToken(string token)
        {
            ...
            var actual = jwtHelper.IsValid(token, new string[] { JwtScopes.Write.ToString() }, issued);
